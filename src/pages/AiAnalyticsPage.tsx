    ...
    <div className="p-4 max-w-[1750px] mx-auto space-y-4 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-lg font-bold text-white">Central AI Analytics & Edge Neural Modules</h1>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-2.5">
                      <div className={`p-2 rounded ${isSelected ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-300'}`}>
                        <Icon className="w-4 h-4" />
                      </div>
                      <div>
                        <div className="text-xs font-bold text-white">{mod.title}</div>
                        <div className="text-[10px] text-slate-400 font-mono">{mod.category}</div>
                      </div>
                    </div>

                    <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-slate-900 text-slate-300 border border-slate-700">
                      {mod.status}
                    </span>
                  </div>

                  <div className="mt-2 flex items-center justify-between text-[11px] font-mono text-slate-400 pt-2 border-t border-slate-850">
                    <span>Target: <strong className="text-slate-300">{mod.latencyTarget}</strong></span>
        ...
        <div className="lg:col-span-7 space-y-3">
          <div className="bg-[#0F172A] border border-slate-800 rounded-lg p-5 space-y-4 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] font-mono text-blue-400 font-bold">{selectedModule.id}</span>
                <h2 className="text-base font-bold text-white">{selectedModule.title}</h2>
                <div className="text-xs text-slate-400 font-mono mt-0.5">{selectedModule.category}</div>
              </div>

              <div className="flex items-center space-x-2">
                <span className="bg-slate-900 border border-slate-700 text-slate-200 px-2.5 py-1 rounded text-xs font-mono">
            ...
            <div className="bg-[#0B1120] border border-slate-800 rounded p-4 space-y-3 font-mono text-xs">
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="bg-slate-900 p-2 rounded border border-slate-800 text-slate-200 mt-1">
                ...
                <div>
                  ...
                  <div className="bg-slate-900 p-2 rounded border border-slate-800 text-emerald-300 font-bold mt-1">
            ...
            {selectedModule.id === 'AI-ANPR-01' && (
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded text-xs space-y-1">
                <div className="font-bold text-white font-mono">Simulated ANPR Protocol:</div>
            ...
            {selectedModule.id === 'AI-FACE-02' && (
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded text-xs space-y-1">
                <div className="font-bold text-white font-mono">Watchlist Matching Protocol:</div>
                <div className="text-slate-400 text-[11px] leading-relaxed">
            ...
            {selectedModule.id === 'AI-OBJ-03' && (
              <div className="p-3 bg-slate-900/60 border border-slate-800 rounded text-xs space-y-2">
                <div className="font-bold text-white font-mono">Supported Class Labels:</div>
                <div className="flex flex-wrap gap-2 text-xs font-mono">
                  {['PERSON', 'VEHICLE', 'BAG', 'MOTORCYCLE'].map((cls) => (
                    <span key={cls} className="bg-slate-800 border border-slate-700 text-slate-200 px-2.5 py-1 rounded">
                      {cls}
                    </span>
                  ))}
                </div>
