  ...
  return (
    <div className="min-h-full py-8 px-4 flex flex-col items-center justify-center max-w-5xl mx-auto space-y-6 font-sans">
      ...
      <div className="text-center space-y-2.5 max-w-2xl">
        <div className="inline-flex items-center space-x-2 px-3 py-1 rounded bg-surface border border-blue-500/40 text-text text-xs font-bold">
        ...
        <div className="bg-surface border border-border rounded p-2.5 text-xs text-text text-center">
          ...
          return (
            <div
              ...
              className={`cursor-pointer rounded-lg p-5 border transition-all duration-150 flex flex-col justify-between space-y-4 relative ${
                isSelected
                  ? 'bg-surface border-blue-500 shadow ring-1 ring-blue-500'
                  : 'bg-surface border-border hover:border-border hover:bg-surface'
              }`}
            >
              {isSelected && (
                <div className="absolute top-3 right-3 bg-blue-600 text-text font-mono text-[9px] font-bold px-2 py-0.5 rounded">
                  ACTIVE PERSONA
                </div>
              )}
              ...
              <div className="space-y-3">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    <h3 className="font-bold text-sm text-text">{config.title}</h3>
                    <div className="text-[10px] font-mono text-text-muted">{config.subtitle}</div>
                  </div>
                </div>

                <div className="bg-bg p-2.5 rounded border border-border text-xs font-mono">
                ...
                <p className="text-xs text-text-muted leading-relaxed font-sans">{config.description}</p>
                ...
                <div className="space-y-1 pt-1.5 border-t border-border">
                  ...
                  {config.permissions.map((perm, idx) => (
                    <div key={idx} className="flex items-center space-x-1.5 text-[11px] text-text-muted font-mono">
              ...
              <button
                ...
                className={`w-full py-2 px-3 rounded text-xs font-mono font-bold flex items-center justify-center space-x-1.5 transition-colors ${
                  isSelected
                    ? 'bg-blue-600 hover:bg-blue-500 text-text'
                    : 'bg-slate-800 hover:bg-slate-700 text-text'
                }`}
              >
