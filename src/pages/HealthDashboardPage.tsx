...
export const HealthDashboardPage: React.FC = () => {
  ...
  return (
    <div className="p-4 max-w-[1750px] mx-auto space-y-4 font-sans">
      {/* Toast Notification */}
      {successToast && (
        <div className="fixed top-12 right-6 z-50 bg-[#0F172A] border border-emerald-500 text-emerald-200 px-4 py-2.5 rounded-lg shadow-2xl flex items-center space-x-2 text-xs font-mono">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-lg font-bold text-white">Camera Network Health & Predictive Maintenance (O&M)</h1>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-xs font-mono">
        <div className="bg-[#0F172A] border border-slate-800 rounded p-3">
          ...
          <span className="text-[10px] text-slate-400">Simulated 30-Day Fleet SLA</span>
        </div>
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded p-3">
          ...
          <span className="text-[10px] text-slate-400">Pre-outage telemetry anomaly</span>
        </div>
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded p-3">
          ...
          <span className="text-[10px] text-slate-400">Down from 4.6h (Target: ≤ 3h)</span>
        </div>
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded p-3">
          ...
          <span className="text-[10px] text-slate-400">Simulated local ticket queue</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-xs font-bold font-mono uppercase text-white">
                CAMERA NETWORK HEALTH • UPTIME BY DISTRICT (%)
              </h3>
              <p className="text-[11px] text-slate-400">Representative 30-day availability across Gujarat districts</p>
            </div>
          ...
          <div className="h-60 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={districtUptimeData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                ...
                />
                <Bar dataKey="uptime" fill="#3B82F6" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-[#0F172A] border border-slate-800 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-400">Average vendor resolution hours from ticket dispatch to close</p>
            </div>
          ...
          <div className="h-60 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mttrTrendData} margin={{ top: 10, right: 20, left: -20, bottom: 10 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#1E293B" />
                <XAxis dataKey="week" tick={{ fill: '#94A3B8', fontSize: 11, fontFamily: 'monospace' }} />
                <YAxis domain={[0, 6]} tick={{ fill: '#94A3B8', fontSize: 11, fontFamily: 'monospace' }} />
                <Tooltip
                  contentStyle={{
                    backgroundColor: '#0B1120',
                    borderColor: '#334155',
          ...
          <table className="w-full text-left text-xs font-sans">
            ...
            <tbody className="divide-y divide-slate-900 font-mono text-[11px]">
              {canonicalAtRisk.map((cam) => (
                <tr key={cam.id} className="hover:bg-slate-850 transition-colors">
                  <td className="p-3 font-bold text-blue-400">{cam.id}</td>
                  <td className="p-3 font-sans">
                    <div className="font-semibold text-white">{cam.locationName}</div>
                  ...
                  <td className="p-3 font-bold text-slate-200">{cam.healthScore}/100</td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-right">
                    <button
                      ...
                      className="px-2.5 py-1 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded text-[10px] transition-colors inline-flex items-center space-x-1"
                    >
      ...
      <div className="bg-[#0F172A] border border-slate-800 rounded-lg overflow-hidden shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-sans">
            ...
            <tbody className="divide-y divide-slate-900 font-mono text-[11px]">
              {workOrders.map((wo) => (
                <tr key={wo.id} className="hover:bg-slate-850 transition-colors">
                  <td className="p-3 font-bold text-amber-400">{wo.id}</td>
                  <td className="p-3 text-blue-400 font-bold">{wo.cameraId}</td>
                  <td className="p-3 text-slate-300">{wo.district}</td>
                  <td className="p-3 font-sans text-slate-200 max-w-sm">{wo.issue}</td>
                  <td className="p-3">
                    <span
                  ...
                  <td className="p-3 text-slate-300 font-sans">{wo.technicianVendor}</td>
                  <td className="p-3 text-slate-400">{wo.createdAt}</td>
                </tr>
              ))}
            </tbody>
          </table>
